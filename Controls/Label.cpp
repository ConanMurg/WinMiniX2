...
{
	NullSet = FALSE;
	NullBkColor = GetSysColor(COLOR_3DFACE);
	NullTextColor = GetSysColor(COLOR_GRAYTEXT);
	LastBkColor = GetSysColor(COLOR_3DFACE);
	LastTextColor = GetSysColor(COLOR_WINDOWTEXT);
...
{
	if (!NullSet) LastTextColor = crText;
	m_crText = crText;
	RedrawWindow();
	return *this;
}
...
{
	if (!NullSet) LastBkColor = crBkgnd;
	if (m_hBrush)
		::DeleteObject(m_hBrush);
...
void CLabel::SetNullColor()
{
	NullSet = TRUE;
	SetBkColor(NullBkColor);
	SetTextColor(NullTextColor);
	NullSet = FALSE;
}

void CLabel::SetLastColor()
{
	NullSet = TRUE;
	SetBkColor(LastBkColor);
