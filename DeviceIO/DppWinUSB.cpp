...
BOOL CDppWinUSB::Initialize_Device(DWORD MemberIndex)
{
	...
	if(bResult) {
		...
		for(int i=0;i<ifaceDescriptor.bNumEndpoints;i++) {
			...
			PipeTimeout[i] = 500;
			//[3]
			bResult = WinUsb_QueryPipe(devInfo.winUSBHandle, 0, (unsigned char) i, &pipeInfo);
			...
			if(pipeInfo.PipeType == UsbdPipeTypeBulk && USB_ENDPOINT_DIRECTION_IN(pipeInfo.PipeId)) {
				...
				WinUsb_SetPipePolicy(devInfo.winUSBHandle, devInfo.bulkInPipe, IGNORE_SHORT_PACKETS, sizeof(unsigned char), &bFalse);  
				WinUsb_SetPipePolicy(devInfo.winUSBHandle, devInfo.bulkInPipe, AUTO_CLEAR_STALL, sizeof(unsigned char), &bTrue);    
				WinUsb_SetPipePolicy(devInfo.winUSBHandle, devInfo.bulkInPipe, ALLOW_PARTIAL_READS, sizeof(unsigned char), &bFalse);    
				WinUsb_SetPipePolicy(devInfo.winUSBHandle, devInfo.bulkInPipe, PIPE_TRANSFER_TIMEOUT, sizeof(unsigned long), &pipeTimeout);
			} else if(pipeInfo.PipeType == UsbdPipeTypeBulk && USB_ENDPOINT_DIRECTION_OUT(pipeInfo.PipeId)) {
				devInfo.bulkOutPipe = pipeInfo.PipeId;
				WinUsb_SetPipePolicy(devInfo.winUSBHandle, devInfo.bulkInPipe, IGNORE_SHORT_PACKETS, sizeof(unsigned char), &bFalse);  
				//WinUsb_SetPipePolicy(devInfo.winUSBHandle, devInfo.bulkOutPipe, SHORT_PACKET_TERMINATE, sizeof(unsigned char), &bTrue);
				WinUsb_SetPipePolicy(devInfo.winUSBHandle, devInfo.bulkOutPipe, SHORT_PACKET_TERMINATE, sizeof(unsigned char), &bFalse);
				WinUsb_SetPipePolicy(devInfo.winUSBHandle, devInfo.bulkOutPipe, AUTO_CLEAR_STALL, sizeof(unsigned char), &bTrue);
				WinUsb_SetPipePolicy(devInfo.winUSBHandle, devInfo.bulkOutPipe, PIPE_TRANSFER_TIMEOUT, sizeof(unsigned long), &pipeTimeout);
...
int CDppWinUSB::CountDP5WinusbDevices(void) 
{
    ...
    HKEY hKey;
    DWORD retCode;
	unsigned char DevicePath[MAXREGBUFFER];
    DWORD cbDevicePath;
