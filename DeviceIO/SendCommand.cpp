...
bool CSendCommand::DP5_CMD(unsigned char Buffer[], TRANSMIT_PACKET_TYPE XmtCmd)
{
	...
	switch (XmtCmd) {
		...
		case XMTPT_SEND_STATUS:
            ...
            POUT.PID2 = PID2_SEND_STATUS;   // send status only
			break;
 ...
 //==========================================================================//Mini-X2
       //======================================================
        ...
        case XMTPT_SEND_TUBE_ILOCK_TABLE_MX2:
            ...
            POUT.LEN = 0;
			break;
        case XMTPT_SEND_WARMUP_TABLE_MX2:
            POUT.PID1 = PID1_REQ_SCOPE_MISC;
            POUT.PID2 = PID2_SEND_MX2_WARMUP_TABLE;
            POUT.LEN = 0;
			break;
        case XMTPT_SEND_TIMESTAMP_RECORD_MX2:
            POUT.PID1 = PID1_REQ_SCOPE_MISC;
            POUT.PID2 = PID2_SEND_MX2_TIMESTAMP_RECORD;
            POUT.LEN = 0;
			break;
        case XMTPT_SEND_FAULT_RECORD_MX2:
			...
			break;
    //--------------------------------------------------------------
        case XMTPT_WRITE_TIMESTAMP_MX2:
			time_t ttNow;				// World Time
			struct tm *tmMX2_Time;		// Local Time
			ttNow = time(NULL);			// Get World Time
			tmMX2_Time = localtime(&ttNow);	// Get Local Time
			tmMX2_Time->tm_year = tmMX2_Time->tm_year - 100;	// MX2 year to starts at 2000
			tmMX2_Time->tm_mon = tmMX2_Time->tm_mon + 1;		// MX2 month to starts at 1
            POUT.PID1 = PID1_VENDOR_REQ;
            POUT.PID2 = PID2_WRITE_MX2_TIMESTAMP;            // write MNX timestamp
            POUT.LEN = 6;
            POUT.DATA[0] = tmMX2_Time->tm_mon;               // binary month
            POUT.DATA[1] = tmMX2_Time->tm_mday;              // binary day
            POUT.DATA[2] = tmMX2_Time->tm_year;              // binary year, 2018=18
            POUT.DATA[3] = tmMX2_Time->tm_hour;              // binary hour
            POUT.DATA[4] = tmMX2_Time->tm_min;               // binary minutes
            POUT.DATA[5] = tmMX2_Time->tm_sec;               // binary seconds
			break;
    //----------------------------------------------------------------
...
//		XMTPT_TEXT_CONFIGURATION_FILTERED
//		XMTPT_TEXT_CONFIGURATION
//		XMTPT_FULL_READ_CONFIG_PACKET
//		XMTPT_SHORT_READ_TEXT_CONFIGURATION
//		XMTPT_READ_TEXT_CONFIGURATION
bool CSendCommand::DP5_CMD_Config(unsigned char Buffer[], TRANSMIT_PACKET_TYPE XmtCmd, CONFIG_OPTIONS CfgOptions)
{
	...
	switch (XmtCmd) {
        case XMTPT_TEXT_CONFIGURATION_FILTERED:
            ...
            POUT.LEN = (unsigned short)lLen;
			break;
        case XMTPT_TEXT_CONFIGURATION:			// bypass any filters
        case XMTPT_TEXT_CONFIGURATION_MX2:           // bypass any filters
            ...
            POUT.LEN = (unsigned short)lLen;
 			break;
         case XMTPT_FULL_READ_TEXT_CONFIGURATION:
			// CONFIG_OPTIONS Needed:
			//		CfgOptions.PC5_PRESENT
			//		CfgOptions.DppType
			strCfg = "";
            ...
            POUT.LEN = (unsigned short)lLen;
			break;
        case XMTPT_READ_TEXT_CONFIGURATION:					// bypass any filters
        case XMTPT_READ_TEXT_CONFIGURATION_MX2:
            ...
            POUT.LEN = (unsigned short)lLen;
			break;
		case XMTPT_SHORT_READ_TEXT_CONFIGURATION:
			strCfg = "";
