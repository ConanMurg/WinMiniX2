...
string CParsePacket::PID2_TextToString(string strPacketSource, unsigned char PID2)
{
	...
	switch (PID2) {
        ...
        case PID2_ACK_BAD_PARAM:	// now msg includes bad parameter
    		...
    		//[] If PIN.LEN > 0 { ' BAD CMD (PID2=7), BAD PARAM (PID2=5), NO PC5 (PID2=11) errors return offending command string
      //      	[] T$ = ""
...
long CParsePacket::ParsePacket(unsigned char P[], Packet_In *PIN)
{
    ...
    if (PIN->STATUS == PID2_ACK_OK) { // no errors
        if ((PIN->PID1 == PID1_RCV_STATUS) && (PIN->PID2 == RCVPT_DPP_STATUS)) { // DPP status
            ParsePkt = preqProcessStatus;
		} else if ((PIN->PID1 == PID1_RCV_STATUS) && (PIN->PID2 == RCVPT_MX2_STATUS)) { // Mini-X2 status
...
//---------------------------------------------------------------------------------- End Mini-X2
        ...
        } else if ((PIN->PID1 == PID1_RCV_SCA) && (PIN->PID2 == RCVPT_SCA)) {
            ParsePkt = preqProcessSCAData;
...
//} else if PIN->PID2 = 7 { ' Config readback response packet
//    txtConfig.Text = ""
//    For X = 0 To PIN.LEN - 1
